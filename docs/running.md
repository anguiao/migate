...
授权码和回调校验使用的 `state` 只保留在内存中。登录过程中的 EOF、Ctrl-C、输入错误或云端及证书失败均不覆盖已有本地凭据，但不能保证旧授权在云端继续有效。同一账号重新登录复用已有中枢客户端标识与私钥；换账号时生成新身份，不复用旧账号证书。

令牌操作失败时显示失败操作、业务码及可解析的内层 OAuth 错误码。`96002` 表示必要参数缺失或格式错误；`96013` 表示授权码无效，需重新执行 `auth login`，使用新授权链接取得新回调。未知错误保留代码，诊断不输出云端原始错误文字或授权参数。
