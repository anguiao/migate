...
fn real_curtain_declares_a_window_covering_endpoint() {
    ...
    .features
    .into_iter()
    .find(|feature| feature.role == FeatureRole::Curtain)
    .unwrap();
...
fn real_vacuum_declares_rvc_clusters_and_ancillary_battery() {
    ...
    .features
    .into_iter()
    .find(|feature| feature.role == FeatureRole::Vacuum)
    .unwrap();
...
async fn subscribe_curtain_position(client: &Matter<'_>, endpoint: u16) -> Result<u32, Error> {
    let paths = [AttrPath::from_gp(&GenericPath::new(
        Some(endpoint),
...
async fn subscribe_rvc_fault(client: &Matter<'_>, endpoint: u16) -> Result<u32, Error> {
    let paths = [EventPath::from_gp(&GenericPath::new(
        Some(endpoint),
